...
export const uploadRestaurantImage = async (formData) => {
    const token = localStorage.getItem('token');
    if (!token) throw new Error("Authentication token missing.");

    try {
        // ✅ FIX: Using the defined API_URL variable
        const response = await fetch(`${API_URL}/restaurants/me/image`, {
            method: 'PUT',
            headers: {
...
        const data = await parseJSON(response); // Use the existing parser

        if (!response.ok) {
            throw new Error(data.error || "Image upload failed on server.");
        }

        return data; // Returns { message, image: newImageUrl }
